...
namespace SilentEye {
	...
	m_capacityMap.clear();
        ...
        if (!preferedFormat.isEmpty()) {
            int index = formatComboBox->findText(preferedFormat);
            if (index != -1)
                formatComboBox->setCurrentIndex(index);
            else
                m_logger->debug("Cannot find format index for " + preferedFormat);
        }
