...
namespace SilentEye {
    ...
    void OptionDialog::setMessage(const QString& message)
    {
        m_message = message;
    }

    void OptionDialog::setMessageFile(const QString& message_file)
    {
        m_filePath = message_file;
    }
    ...
    int OptionDialog::dataSizeRemaining(FormatModuleInterface* module, QPointer<Media> media) {
        ...
        if (m_filePath == "" || !QFile::exists(m_filePath))
        {
            EncodedData data(msgTextEdit->toPlainText(), currentCharset(),
                             compressCheckBox->checkState()!=Qt::Unchecked);
            size = data.size();
        }
        else
        {
            QFile file(m_filePath);
            EncodedData data(file,
                             compressCheckBox->checkState()!=Qt::Unchecked);
            size = data.size();
        }
