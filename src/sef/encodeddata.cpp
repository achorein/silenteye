...
namespace SilentEyeFramework {
    ...
    /*!
        ...
        \param compressionLevel Specify whether the data should be compressed
    */
    EncodedData::EncodedData(Data::DataFormat format, int compressionLevel)
        : _compressed(compressionLevel), _compressionLevel(compressionLevel)
    {
        _swap = 2;
    ...
    /*!
        ...
        \param compressionLevel Specify whether the data should be compressed
    */
    EncodedData::EncodedData(const QByteArray bytes, Data::DataFormat format, int compressionLevel)
        : _compressed(compressionLevel), _compressionLevel(compressionLevel)
    {
        _swap = 2;
        _andOperator = andOperator(_swap);
        ...
        if(format == Data::F_UNDEF) {
            ...
            if (_compressed)
                _buffer = qCompress(_data->toByteArray(), _compressionLevel);
            else
    ...
    /*!
        ...
        \param compressionLevel Specify whether the data should be compressed
    */
    EncodedData::EncodedData(const quint32 value, int compressionLevel)
        : _compressed(compressionLevel), _compressionLevel(compressionLevel)
    {
        _swap = 2;
        _andOperator = andOperator(_swap);
    ...
    /*!
        ...
        \param compressionLevel Specify whether the data should be compressed
    */
    EncodedData::EncodedData(const QString& str, Data::DataFormat format, int compressionLevel)
        : _compressed(compressionLevel), _compressionLevel(compressionLevel)
    {
        _swap = 2;
        _andOperator = andOperator(_swap);
    ...
    /*!
        ...
        \param compressionLevel Specify whether the data should be compressed
    */
    EncodedData::EncodedData(QFile& file, int compressionLevel)
        : _compressed(compressionLevel), _compressionLevel(compressionLevel)
    {
        _swap = 2;
        _andOperator = andOperator(_swap);
    ...
    /*!
        \param compress new compression level (0:no compression, 9:higher compresssion)
        \param force Indicate that we have to change current data
     */
    void EncodedData::setCompressed(const int compressLevel, const bool force)
    {
        bool compress = (compressLevel > 0 && compressLevel < 10);

        if(force)
        {
            if(!_compressed && compress)
            {
                _buffer = qCompress(_buffer, compressLevel);
            }
        ...
        _compressionLevel = compressLevel;
    }
